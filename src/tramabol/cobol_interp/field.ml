...
let binary_field ~digits ~neg ~byte_size data_ptr =
  let sign =
    if neg
    then Some { sign_position = Leading; sign_separate = true }
    else None
  in
...
let int32_field ~digits i =
  binary_field ~digits ~neg:(i < 0) ~byte_size:4 @@
  CPtr.create ~default:(S32.of_int_unsafe i) SInt32

let int64_field ~digits i =
  binary_field ~digits ~neg:Int64.(compare i zero < 0) ~byte_size:8 @@
